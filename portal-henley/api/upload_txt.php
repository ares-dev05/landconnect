...
if (!isset($_SERVER['PHP_AUTH_USER'])) {
    ...
    if ($userdetails!=null) {
        ...
        // verify if the upload field is present
        if (isset($_POST) && isset($_POST[UPLOAD_FIELD]) && strlen($_POST[UPLOAD_FIELD])) {
            // attempt to upload the file to the bucket
            if (PmPlan::metaUpload($userInfo, $_POST[UPLOAD_FIELD])) {
                // return success
                endCall('Upload succeeded', true);
            }   else {
                endCall(
                    'Upload failed due to an unexpected error. Please try again later or contact Landconnect support.',
                    false,
                    true
                );
            }
