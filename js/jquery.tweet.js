...
(function (factory) {
	...
	$.fn.tweet = function(o){
		var s = $.extend({
			modpath: "./twitter_feed/",                     // [string]   relative URL to Tweet.js mod (see https://github.com/StanScates/Tweet.js-Mod)
...
jQuery(window).load(function(){   
	jQuery(document).ready(function($){ 
	  ...
	  if ($('#feed').length) {
		$('#feed').tweet({
				username: 'OriginThemez',    
				join_text: 'auto',
				avatar_size: 0,
				count: 4
			});
			
		  $('#feed').find('ul').addClass('slides');
		  $('#feed').contents().wrapAll('<div class="flexslider">');
		  };
	});
});	