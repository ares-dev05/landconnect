...
var PORTAL_APIPATH   = getSitePath() + "portal/api/";
var PORTAL_FORMSPATH = getSitePath() + "portal/forms/";

var STATS_APIPATH = getSitePath() + "statistics/api/";

var BILLING_API = getSitePath() + "billing/api/index.php";
...
function getSitePath() {
	...
	return scriptPath.substr(0, scriptPath.lastIndexOf( '/js' )+1 );
}
...
function formatDate1(stamp) {
	...
	formatted['date'] = dateTime.toString("d/MMM/yy");
	formatted['day' ] = dateTime.toString("ddd");
...
function alertWidgetResponse(widget_id, data){
	var alertHTML = "";
	jQuery.each(data, function(alert_idx, alert_message) {
		if (alert_message['type'] == "success"){
			alertHTML += "<div class='alert alert-success'>" + alert_message['message'] + "</div>";
...
function processJSONResult(result) {
	if (!result) {
		...
		try {
			...
			addAlert("danger", "Oops, looks like our server might have goofed.  If you're an admin, please check the PHP error logs. "+result);
            addAlert("danger", result);
			return {"errors": 1, "successes": 0};
		}	
	}
}
...
function getUserLabel( uid, userName ) {
    console.log("getUserLabel "+uid+", "+userName);
